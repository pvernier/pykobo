...
def trim_columns_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    '''
    def trim_ws(x): return x.strip() if isinstance(x, str) else x
    def trim_pt(x): return x.strip('.') if isinstance(x, str) else x
    df_temp = df.applymap(trim_ws)
    return df_temp.applymap(trim_pt)


def reconcile_columns(df: pd.DataFrame, col1: str, col2: str, new_column: str, criteria=float("nan")) -> None:
    '''
...
def reconcile_columns_append(df: pd.DataFrame, col1: str, col2: str, new_column: str, to_replace: str) -> None:
    '''TODO
